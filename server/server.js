...
async function generateQuestions2(req, res) {
    ...
    try {
        ...
        doc.on("end", async () => {
            ...
            await createPacket(client, pdfData, studentId, ansKey);
        ...
        const ansKey = await editPdf(result, doc);
...
async function generateTemplateQuestions(client, objectives) {
    ...
    for (const obj of objectives) {
        ...
        // console.log(result);
        allQuestions = allQuestions.concat(await generateNTemplateQuestions(result, 10));
    }
...
async function createPacket(client, packet, student, ansKey) {
    console.log("Creating packet");
    const response = await client.db("m2m_math_db").collection("packets").insertOne({ content : packet, student_id : ObjectId.createFromHexString(student), answer_key : ansKey, date_created : new Date()});
